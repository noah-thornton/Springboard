...
SELECT membername, facilityname, cost FROM(
	SELECT CONCAT(m.firstname, ' ', m.surname) AS membername, 				f.name AS facilityname,
		CASE 
    		WHEN m.memid = 0 THEN f.guestcost * b.slots
            ELSE f.membercost * b.slots END AS cost
...
SELECT facilityname, revenue FROM(
        SELECT f.name AS facilityname, sum(b.slots *
            CASE 
...
SELECT f.name AS facilityname, m.surname || ', ' || m.firstname AS membername, SUM(b.slots) AS usage_amount
	FROM `Members` AS m
		INNER JOIN `Bookings` AS b
			ON m.memid = b.memid
    		INNER JOIN `Facilities` AS f
...
ORDER BY facilityname, usage_amount DESC;

/* Q13: Find the facilities usage by month, but not guests */

SELECT f.name AS facilityname, strftime('%m', starttime) AS month, SUM(b.slots) AS usage_amount
FROM `Members` AS m
	INNER JOIN `Bookings` AS b
		ON m.memid = b.memid
    INNER JOIN `Facilities` AS f
...
ORDER BY month, facilityname ASC;
